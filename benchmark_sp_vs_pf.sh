...
#   CONTAINER_IMAGE, CONTAINER_WORKDIR, HF_CACHE_DIR, CONTAINER_MOUNTS
...
HF_CACHE_DIR="${HF_CACHE_DIR:-/scratch/fq9hpsac/fq9hpsacuser05/huggingface}"
# Default mounts: project root -> workdir, and huggingface cache
CONTAINER_MOUNTS="${CONTAINER_MOUNTS:-${SCRIPT_DIR}:${CONTAINER_WORKDIR},${HF_CACHE_DIR}:${HF_CACHE_DIR}}"
...
dispatch Single       1 8000 --vae-patch-parallel-size=1
dispatch PF-4         4 8001 --pipeline-parallel-size=4 --enable-pipefusion --pipefusion-split-dim=temporal --vae-patch-parallel-size=4
dispatch RPF-4        4 8002 --pipeline-parallel-size=4 --enable-pipefusion --enable-rotational-pipefusion --pipefusion-split-dim=temporal --vae-patch-parallel-size=4
dispatch SP-4         4 8003 --usp 4 --vae-patch-parallel-size=4
dispatch PF-8         8 8004 --pipeline-parallel-size=8 --enable-pipefusion --pipefusion-split-dim=temporal --vae-patch-parallel-size=8
dispatch RPF-8        8 8005 --pipeline-parallel-size=8 --enable-pipefusion --enable-rotational-pipefusion --pipefusion-split-dim=temporal --vae-patch-parallel-size=8
dispatch SP-8         8 8006 --usp 8 --vae-patch-parallel-size=8
dispatch PF-4xCFG2    8 8007 --pipeline-parallel-size=4 --enable-pipefusion --pipefusion-split-dim=temporal --cfg-parallel-size=2 --vae-patch-parallel-size=8
dispatch RPF-4xCFG2   8 8008 --pipeline-parallel-size=4 --enable-pipefusion --enable-rotational-pipefusion --pipefusion-split-dim=temporal --cfg-parallel-size=2 --vae-patch-parallel-size=8
dispatch SP-4xCFG2    8 8009 --usp 4 --cfg-parallel-size=2 --vae-patch-parallel-size=8

echo
if [[ "${USE_SLURM}" == "1" ]]; then
    echo "=== all jobs submitted ==="
    echo "JobIDs:          ${JOBIDS_FILE}"
    echo "Track:   squeue -u \$USER --name=${JOB_PREFIX}_Single,${JOB_PREFIX}_PF-4,${JOB_PREFIX}_RPF-4,${JOB_PREFIX}_SP-4,${JOB_PREFIX}_PF-8,${JOB_PREFIX}_RPF-8,${JOB_PREFIX}_SP-8,${JOB_PREFIX}_PF-4xCFG2,${JOB_PREFIX}_RPF-4xCFG2,${JOB_PREFIX}_SP-4xCFG2"
