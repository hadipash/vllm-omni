...
ENABLE_ROTATIONAL_PIPEFUSION=0

while (($#)); do
    case "$1" in
        --enable-rotational-pipefusion)
            ENABLE_ROTATIONAL_PIPEFUSION=1
            shift
            ;;
...
echo "[${LABEL}] request PipeFusion: warmup_steps=${PIPEFUSION_WARMUP_STEPS:-<runtime-default>} split_dim=${PIPEFUSION_SPLIT_DIM:-<runtime-default>} rotational=${ENABLE_ROTATIONAL_PIPEFUSION}"
...
run_request() {
    ...
    if [[ "${ENABLE_ROTATIONAL_PIPEFUSION}" == "1" ]]; then
        curl_form+=(-F "enable_rotational_pipefusion=true")
    fi
...
[[ " ${EXTRA_FLAGS[*]} " == *"--enable-rotational-pipefusion"* ]] && strategy="RPF"
cfgp=1
